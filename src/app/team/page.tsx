...
export default function TeamPage() {
  ...
  async function handleInvite() {
    ...
    setSending(true);
    try {
      // Get workspace ID from current members (assuming all members belong to the same workspace)
      const WsRes = await fetch("/api/workspaces");
      const WsData = await WsRes.json();
      const WsId = WsData.data?.[0]?.id;
      if (!WsId) {
        toast.error("No Workspace not found!");
        return;
      }
      ...
      if (data.emailSent) {
        ...
        toast.success(`Invitation created for ${inviteEmail} (email skipped)`);
      }
      setInviteEmail("");
      setShowInvite(false);
    } catch (error) {
        ...
        {[
          ...
          >
            {loading ? (
              ...
              <p className={cn("text-2xl font-black mb-0.5", s.color)}>
                {s.value}
              </p>
            )}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#7c7ca8] truncate max-w-28">
                        {member.user.email}
                      </p>
                    </div>
                  </div>
