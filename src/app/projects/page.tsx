...
import {
  ...
  X, Check, Loader2, Zap, Lock, Globe
} from "lucide-react"
...
const PROJECTS = [
  {
    id: "1", name: "DevBoard", color: "#8b5cf6", icon: "⚡",
    description: "Main task management platform",
    members: 8, tasks: 42, done: 28, status: "active",
    isPublic: false, updatedAt: "2025-04-04T10:00:00",
  },
  {
    id: "2", name: "Alpha App", color: "#06b6d4", icon: "🚀",
    description: "Mobile app for team collaboration",
    members: 5, tasks: 18, done: 10, status: "active",
    isPublic: true, updatedAt: "2025-04-03T15:00:00",
  },
  {
    id: "3", name: "API Gateway", color: "#22c55e", icon: "🔌",
    description: "Microservices API gateway",
    members: 3, tasks: 25, done: 25, status: "done",
    isPublic: false, updatedAt: "2025-04-01T10:00:00",
  },
  {
    id: "4", name: "Design System", color: "#f59e0b", icon: "🎨",
    description: "Shared UI component library",
    members: 4, tasks: 33, done: 15, status: "active",
    isPublic: true, updatedAt: "2025-04-02T12:00:00",
  },
]
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Projects", value: projects.length,                                           color: "text-violet-400", bg: "bg-violet-500/10", border: "border-violet-500/20" },
          { label: "Active",         value: projects.filter(p => p.status === "active").length,        color: "text-cyan-400",   bg: "bg-cyan-500/10",   border: "border-cyan-500/20"   },
          { label: "Completed",      value: projects.filter(p => p.status === "done").length,          color: "text-green-400",  bg: "bg-green-500/10",  border: "border-green-500/20"  },
          { label: "Total Tasks",    value: projects.reduce((s, p) => s + p.tasks, 0),                color: "text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-500/20" },
        ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((project, i) => {
          const pct = Math.round((project.done / project.tasks) * 100) || 0
          return (
            ...
            >
              ...
              {/* Description */}
              <p className="text-xs text-[#7c7ca8] mb-4 line-clamp-2 leading-relaxed">
                {project.description}
              </p>
