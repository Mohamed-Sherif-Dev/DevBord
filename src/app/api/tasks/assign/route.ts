...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      dueDate,
      projectName,
      taskId,        // ✅ لو في taskId — نعمل assign في الـ DB
    } = await req.json()

    if (!assigneeEmail) {
      return Response.json({ message: "assigneeEmail required" }, { status: 400 })
    }

    // ✅ دور على الـ User بالإيميل وعمل assign في الـ DB
    if (taskId) {
      ...
      if (assignee) {
        ...
        // ✅ إبعت Notification للـ assignee
        await prisma.notification.create({
          data: {
            userId:  assignee.id,
            type:    "TASK_ASSIGNED",
            title:   "New Task Assigned!",
            message: `${session.user?.name} assigned you "${taskTitle}"`,
            link:    "/my-tasks",
          }
        })
      }
    ...
    if (!process.env.MAIL_USER || !process.env.MAIL_PASS) {
      console.log("⚠️ SMTP not configured")
      return Response.json({ message: "Task assigned", emailSent: false })
    }
    ...
    await transporter.sendMail({
      ...
      <div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto;
        ...
        <div style="background:linear-gradient(135deg,#7c3aed,#06b6d4);padding:32px;text-align:center;">
          ...
          <p style="color:rgba(255,255,255,0.7);font-size:13px;margin:6px 0 0">DevBoard</p>
        </div>
        <div style="padding:32px;">
          <p style="color:#a0a0c0;font-size:15px;margin-bottom:24px;">
            Hey <strong style="color:#f0f0ff">${assigneeName || "there"}</strong>! 👋<br/>
            <strong style="color:#8b5cf6">${session.user?.name}</strong> assigned you a new task.
          </p>
          <div style="background:#1a1a2e;border:1px solid #2a2a4a;border-radius:12px;padding:20px;margin-bottom:24px;">
            <h2 style="color:#f0f0ff;font-size:18px;font-weight:800;margin:0 0 12px;">${taskTitle}</h2>
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <span style="background:#2a1a4a;color:#a78bfa;padding:4px 12px;border-radius:8px;font-size:12px;">
                📁 ${projectName || "DevBoard"}
              </span>
              <span style="background:${(priorityColor[priority] || "#8b5cf6")}20;
                color:${priorityColor[priority] || "#8b5cf6"};
              ...
              ${dueDate ? `<span style="background:#1e1e3a;color:#94a3b8;padding:4px 12px;border-radius:8px;font-size:12px;">📅 ${dueDate}</span>` : ""}
            </div>
          </div>
          <a href="${process.env.APP_URL || "http://localhost:3000"}/my-tasks"
            style="display:block;text-align:center;background:linear-gradient(135deg,#7c3aed,#06b6d4);
    ...
    return Response.json({ message: "Task assigned & email sent!", emailSent: true })
